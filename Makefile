...
test: manifests generate fmt vet setup-envtest kustomize ## Run tests.
	@mkdir -p config/crd/kustomized
	"$(KUSTOMIZE)" build config/crd -o config/crd/kustomized/
...
KIND_CLUSTER ?= initjob-operator-test-e2e
KIND_CLUSTER_DEV ?= initjob-operator-dev
...
.PHONY: kind-delete
kind-delete: ## Delete the Kind cluster for development/testing
...
.PHONY: kind-deploy
kind-deploy: kind-load-image install deploy ## Build, load image into Kind, and deploy controller
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name initjob-operator-builder
	$(CONTAINER_TOOL) buildx use initjob-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm initjob-operator-builder
	rm Dockerfile.cross
